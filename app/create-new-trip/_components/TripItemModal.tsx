...
export type ModalItem = {
    title: string;
    subtitle: string; // Address or short desc
    imageUrl: string;
    images?: string[]; // Array of images if available
    rating?: number;
    price?: string;
    description?: string;
    type: 'hotel' | 'activity';
    context?: string; // "Recommended Hotel" or "Activity"
    duration?: string;
    bestTime?: string;
    geo_coordinates?: {
        lat: number;
...
function TripItemModal({ isOpen, onClose, item }: Props) {
    ...
    // Convex Hooks
    const savePlace = useMutation((api as any).savedPlaces.SavePlace);
    const removePlace = useMutation((api as any).savedPlaces.RemovePlace);
    const updateNote = useMutation((api as any).savedPlaces.UpdateNote);
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            if (userDetail && item && note.trim() !== (savedPlace?.notes || "")) {
                ...
                if (note.trim()) {
                    // Start saving
                    const userId = userDetail?.email || userDetail?._id;
                    if (!savedPlace) {
                        ...
                        savePlace({
                            ...
                            updateNote({ userId: userId, placeName: item.title, note: note });
                        });
                    } else {
    ...
    const handleToggleSave = async (e: React.MouseEvent) => {
        ...
        if (savedPlace) {
            await removePlace({ userId: userId, placeName: item!.title });
            setNote("");
            ...
            <div className="flex flex-col max-h-[85vh] h-full bg-white dark:bg-zinc-950">
                ...
                >
                    ...
                    <div className="w-full h-full relative">
                        <div className="absolute inset-0 transition-opacity duration-500 ease-in-out">
                            <Image
                                src={item.images && item.images.length > 0 ? item.images[activeIndex] : (item.imageUrl || '/placeholder.jpg')}
                                alt={item.title}
                                fill
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
                    ...
                    <div className="flex justify-between items-start gap-4">
                        <div>
                            {item.context && (
                                <p className="text-xs font-bold text-primary mb-1 uppercase tracking-wider text-purple-400">
                                    {item.context}
                                </p>
                            )}
                            ...
                            <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
                                <MapPin className="h-4 w-4 text-primary text-purple-500" /> {item.subtitle}
                            </p>
                        </div>
                        {item.rating && (
                        ...
                        {item.type === 'hotel' ? (
                            <>
                                <div className="p-3 bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer group/chip">
                                    ...
                                    <p className="text-lg font-bold text-primary group-hover/chip:scale-105 transition-transform origin-left text-purple-400">{item.price}</p>
                                </div>
                                <div className="p-3 bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer group/chip">
                ...
                <div className="sticky bottom-0 p-4 border-t border-gray-100 dark:border-zinc-800 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md flex flex-col gap-3 z-10 shrink-0">
                    <div className="flex gap-3">
                        ...
                        <Link
                            ...
                            className="flex-[2]"
                        >
                            <Button className="w-full h-12 text-base rounded-xl font-semibold shadow-lg shadow-blue-500/20 bg-linear-to-r from-purple-500 to-fuchsia-600 hover:to-purple-500 transition-all duration-300 bg-size-[200%_auto] hover:bg-right">
